...
export function AppShell({
  ...
  return (
    <NotificationProvider userId={currentUser?.id} organizationId={currentOrganization.id}>
      <main className="min-h-screen px-3 pb-24 pt-3 text-slate-950 sm:px-5 sm:pb-6 lg:px-6">
        ...
        <nav className="fixed inset-x-3 bottom-3 z-30 flex items-end gap-1 rounded-3xl border border-slate-200 bg-white/95 p-1.5 shadow-soft backdrop-blur lg:hidden">
          ...
          <BottomTab
            ...
            icon={<BellIcon />}
            {...(unreadNotificationCount > 0
              ? { badge: String(unreadNotificationCount > 9 ? '9+' : unreadNotificationCount) }
              : {})}
          />
          <BottomTab
            href="/settings"
...
function BottomTab({
  ...
  return (
    ...
    >
      ...
      {active ? <span className="mt-0.5 text-[0.62rem] font-bold">{label}</span> : null}
    </Link>
  );
}
